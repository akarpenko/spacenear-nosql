...
ClusterMarker.prototype._clusterMarker=function($clusterGroupIndexes){
  ...
  var icon = this.clusterMarkerIcon;
  var vehicle_index = -1;
  var vehicle_mismatch = false;
    ...
    if (vehicle_index != -1 && $marker.vehicle_index != vehicle_index) {
      vehicle_mismatch = true;
    }
    vehicle_index = $marker.vehicle_index;
	}

  if (!vehicle_mismatch) {
    icon = new GIcon(G_DEFAULT_ICON);
    icon.image = "icons/iconb" + (vehicle_index + 1) + ".png";
  }

	$clusterMarker=$newClusterMarker($clusterGroupBounds.getCenter(), icon, this.clusterMarkerTitle.replace(/%count/gi, $clusterGroupIndexes.length));
